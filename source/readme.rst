...
- ``--no-verify`` - Skip checksum generation (checksums are generated by default)
...
- ``--no-verify`` - Skip checksum verification (verification is enabled by default)
- ``--keep-chunks`` - Don't delete chunks after successful reconstruction (see FR-TRANSFER-014)
...
.. Note: The Principles link above uses an external URL because it lives in the
   parent documentation project. When built as part of the full site, intersphinx
   resolves cross-project references automatically.
